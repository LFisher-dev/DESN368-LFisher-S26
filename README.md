# About This Repository
This repository documents my journey through DESN368, aka Code + Design, where I learned web building with HTML and CSS in addition to udnerstanding how the web works at the source level.

[portfolio](https://lfisher-dev.github.io/DESN368-LFisher-S26/index.html)

[learning log](https://lfisher-dev.github.io/DESN368-LFisher-S26/learning-log.html) 

# Design Direction
For my portfolio, I wanted to pursue something that was both creative and inviting while also staying professional

header Font: Lemonada Regular
---
Body Font: cabin regular
---
Primary color: #962D45
---
Accent color: 
---
Background: #FFFAFB

[View Figma file here](figma.com/design/P38rbTuxRmbputwPIIfXKh/Portfolio-System_Lee-Fisher?m=dev)
# Featured Projects
### The Recipe
a project where we had to create a recipe page using CSS (in place of inline styling) for the first time
[link]

### Tribute Site
a project where we had to create a website dedicated to a historical person of our choosing
[link]

### Field Guide
a project where we created a field guide based on a game or system of our choosing while utilizing responsive design
[link]

## Product Landing Page
a project where we created a website for a product or subscription service while utilizing CSS animation and contact forms
[link]

# Technical Skills
- semantic HTML 5
- CSS layout (flexbox/grid)
- Responsive Design with media queries
- CSS custom properties or variables
- CSS transitions
- Version control with Git & Github

# Archive
[course log]
[exercises]

# Credits & Citations
### Assets

### Tutorials & Resources
While I can't point to a specific tutorial that's helped me, the biggest resource I've used is w3schools, especially their "try it" feature as a way to both see the code and see how it operates on a website. One specific example is their [horizontal navigation](https://www.w3schools.com/css/css_navbar_horizontal.asp) in helping me figure out how to get the navigation to work horizontally.

### AI Assistance
I do want to disclose that while I have used AI assistance for coding later projects, the most usage has been the "ask" model of Github's copilot helping point me in the right direction for my code. I highlight specific instances below:

- Copilot Ask to help with mobile navigation in week 6
- Copilot Ask to help with hero section layout (text & image) in week 9
- Copilot Ask to help with debugging CSS in week 10

# Reflection
[Read full reflection on Portfolio Site]
